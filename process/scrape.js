...
function get_rating($) {
  try {
    const rating = $("i.a-icon.a-icon-star.a-star-4-5").text().trim();
    return rating;
  } catch (error) {
    try {
...
function get_review_count($) {
  try {
    const review_count = $("span#acrCustomerReviewText").text().trim();
    return review_count;
  } catch (error) {
    return "";
  }
...
function get_availability($) {
  try {
    const available = $("div#availability span").text().trim();
    return available;
  } catch (error) {
    ...
    .then((response) => {
      ...
      csvWriter
        ...
        .then(() => {
          ...
          res.send("Scraped data saved in CSV file");
        })
        .catch((error) => {
          console.log(error);
